 ...
 * 1번지는 setter개념으로서 0번지의 상태겂을 업데이트하도록 사전에 설정되어있습니다.
 * 
 * 그렇기 때문에 저희가 따로 setter 형태로 함수를 정의하지 않더라고
...
// const[count,setCount] = userState(initialValue);
// 70번 라인 까지의 count = initialValue;
// setCount(newValue);
 ...
 * 상태 업데이트는 비동기적으로 일어납니다. 새 상태값이 현재 상태 값에 따라 달라진 수 있다는 문제가 있습니다.
 */

// 비동기적으로 상태가 업데이트 된다는 사례를 증명하는 거고
// 리액트 컴포넌트 내에 상태를 사용하는 useState를 적용
// 그 useState가 자바스크립트 식 함수정의에 사용되었습니다.
...
// 내일 그러면 <p> 태그와 <button> 태그로 이루어진 완벽한 형태의 Counter 컴포넌트를 작성하여 실행하는 것 부터 수업을 시작할 예정
// 완벽한 형태의 Counter 컴포넌트를 작성하여 실행하는 것 부터 수업을 시작할 예정
...
function Counter() {
  ...
  return(
    ...
    <button onClick={() => setCount(count => count+1)}>눌르시오 ㅋㅋ</button>
    </>
  )
}
